...
global Session;
global Trial;
Session.codeUsed='P019A Remedial Vert'; %name of the code used for this session

Session.ITI=3; %Length of ITI
Session.stimTime=13; %Length of time (in secs) that the stim is on the screen
...
Session.expName='P019A Remedial Vert'; %Name of the experiment
...
function RunStim() %function that runs the training stim
    ...
    if Session.trialList(Trial.curTrial) == 0; %see session setup
        dir = 'C:\Users\Admin\Desktop\temp\JULIA\VertSmallCorrect\'; %then use this directory, which will only draw correct stim
        Trial.type='V Correct'; %specifies  the trial type
        Trial.rewardable=1;
    elseif Session.trialList(Trial.curTrial) == 1;
        dir = 'C:\Users\Admin\Desktop\temp\JULIA\VertSmallMajor\'; %then use this directory, which only draw major rule violations
        Trial.type='V Major'; %labels this trial type
        Trial.rewardable=0;
    else
        dir = 'C:\Users\Admin\Desktop\temp\JULIA\VertSmallMinor\'; %makes the other option in the 1 if loop this directory
        Trial.type='V Minor';
        Trial.rewardable=0;
