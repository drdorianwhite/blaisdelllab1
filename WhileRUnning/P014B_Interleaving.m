...
global Session;
global Trial;
Session.codeUsed='P014B_Interleaving'; %name of the code used for this session
...
Session.totalTrialNumber=112; %total number of trials
Session.sessionLength=100*100; %session lqength determined by number of each trial type??
Session.trialTypes=2; %The trial types, correqctqor incorrect, for multiple of each add then subdevide withini trial function.
...
global Session;
    ...
    Session.trialList(Session.trialList(:,2)<=(Session.VertNum),1)=1;
    Session.trialList(Session.trialList(:,2)<=((Session.VertNum/2)+Session.HortNum),1)=2;
    Session.trialList(Session.trialList(:,2)<=((Session.VertNum/4)+Session.HortNum),1)=3;
    
    Session.trialList(Session.trialList(:,2)<=(Session.HortNum),1)=4; %For half correct makes half 1
    Session.trialList(Session.trialList(:,2)<=(Session.HortNum/2),1)=5; %Making half of the remaining half equal to 2 (type 1 of incorrect)
    Session.trialList(Session.trialList(:,2)<=(Session.HortNum/4),1)=6; %For half correct makes half 1
...
%    if Session.level==1;
        ...
        Trial.type='V Correct'; %labels this trial type
        Trial.rewardable=1;
...
elseif Session.trialList(Trial.curTrial) == 2;
        ...
        Trial.type='V Minor'; %specifies  the trial type
        Trial.rewardable=0;
...
elseif Session.trialList(Trial.curTrial) == 3;
        ...
        Trial.type='V Major'; %specifies  the trial type
        Trial.rewardable=0;
else 
