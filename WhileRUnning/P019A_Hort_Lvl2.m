...
Session.expName='P019A Hort 2'; %Name of the experiment
Trial.rule='Hort 2'; %Rule shown if changees by day read in under getbirddata
...
function RunStim() %function that runs the training stim
    ...
    if Session.trialList(Trial.curTrial) == 0; %see session setup
        dir = 'C:\Users\Admin\Desktop\JULIA\HortCorrectLevel2\'; %then use this directory, which will only draw correct stim
        Trial.type='H Correct'; %specifies  the trial type
        Trial.rewardable=1;
    elseif Session.trialList(Trial.curTrial) == 1;
        dir = 'C:\Users\Admin\Desktop\JULIA\HortMajorLevel2\'; %then use this directory, which only draw major rule violations
        Trial.type='H Major'; %labels this trial type
        Trial.rewardable=0;
    else
        dir = 'C:\Users\Admin\Desktop\JULIA\HortMinorLevel2\'; %makes the other option in the 1 if loop this directory
        Trial.type='H Minor';
        Trial.rewardable=0;
