...
global Session;
global Trial;
Session.codeUsed='P019aHortProbeSet2'; %name of the code used for this session
...
Session.totalTrialNumber=120; %total number of trials
Session.sessionLength=120*120; %session length determined by number of each trial type??
...
Session.expName='P019A Hort Probe 2'; %Name of the experiment
Trial.rule='Hort'; %Rule shown if changees by day read in under getbirddata
...
Session.trialList=zeros((Session.baseLineNum+Session.probeNum),2); %frame for storing
Session.trialList(:,2)=randperm((Session.baseLineNum+Session.probeNum)); %order randomizing used for balancing trial type
Session.trialList(Session.trialList(:,2)<=(Session.baseLineNum),1)=1;
   Session.trialList(Session.trialList(:,2)<=((Session.baseLineNum/2)+Session.probeNum),1)=2;
   Session.trialList(Session.trialList(:,2)<=((Session.baseLineNum/4)+Session.probeNum),1)=3;
%Session.trialList(Session.trialList(:,2)<=(((Session.baseLineNum)/Session.trialTypes)+Session.probeNum),1)=1; %For half correct makes half 1
%Session.trialList(Session.trialList(:,2)<=(((Session.baseLineNum+Session.probeNum)/(Session.trialTypes*2)+Session.probeNum)),1)=2; %Making half of the remaining half equal to 2 (type 1 of incorrect)
Session.trialList(Session.trialList(:,2)<=(Session.probeNum),1)=4;
Session.trialList(Session.trialList(:,2)<=(Session.probeNum/2),1)=5;
Session.trialList(Session.trialList(:,2)<=(Session.probeNum/4),1)=6;
a=Session.trialList;
...
global Trial;
global Session;
while (Trial.curTrial<Session.baseLineNum+Session.probeNum) &&((GetSecs()-Session.startSessionTime)<Session.sessionLength) %run this code when the current trial is less than the total number of trials +1
...
if Session.trialList(Trial.curTrial) ==4
    dir = 'C:\Users\Admin\Desktop\temp\JULIA\HortCorrectProbe1P014C\';
    Trial.type = 'Probe Correct Hort';
    Trial.rewardable=0;
    
elseif Session.trialList(Trial.curTrial) ==5
    dir = 'C:\Users\Admin\Desktop\temp\JULIA\HortMajorProbe1P014C\';
    Trial.type = 'Probe Major Hort';
    Trial.rewardable=0;
elseif Session.trialList(Trial.curTrial) ==6
    dir = 'C:\Users\Admin\Desktop\temp\JULIA\HortMinorProbe1P014C\';
    Trial.type= 'Probe Minor Hort';
    Trial.rewardable=0;
    
elseif Session.trialList(Trial.curTrial) == 1; %see session setup
     dir = 'C:\Users\Admin\Desktop\temp\JULIA\HortCorrectLvlOneP014C\'; %makes the other option in the 1 if loop this directory
       Trial.type='H Correct';
       Trial.rewardable=1;

elseif Session.trialList(Trial.curTrial) == 2;
%    if Session.level==1;
        dir = 'C:\Users\Admin\Desktop\temp\JULIA\HortMajorLvlOneP014C\'; %then use this directory, which only draw major rule violations
        Trial.type='Major H'; %labels this trial type
        Trial.rewardable=0;

elseif Session.trialList(Trial.curTrial) == 3;
%    if Session.level==1;
        dir = 'C:\Users\Admin\Desktop\temp\JULIA\HortMinorLvlOneP014C\'; %then use this directory, which only draw major rule violations
        Trial.type='Minor H'; %labels this trial type
        Trial.rewardable=0;
else 
         dir = 'C:\Users\Admin\Desktop\temp\JULIA\HortCorrectLvlOneP014C\'; %then use this directory, which will only draw correct stim
        Trial.type='H Correct'; %specifies  the trial type
        Trial.rewardable=1;
